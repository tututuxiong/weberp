...
def genDatafromSql(orderId):
    ...
    for saleItem_sql in order.salesitem_set.all():
        ...
        print(product_sql.name,1)
        for checkOutrecord_sql in saleItem_sql.checkoutrawmatreporecord_set.all():
            material_sql = RawMat.objects.get(pk=checkOutrecord_sql.rawMaterial_id)
            ...
            query_set = RawMatOrderItem.objects.filter(rawMat=material_sql,status="已关闭")
            if (query_set.count() > 0):
                RawMatOrderItem_sql = query_set.reverse()[0]
                if RawMatOrderItem_sql:
                    checkoutRecord_materialPrice.append(checkOutrecord_sql.num*RawMatOrderItem_sql.est_total_price/RawMatOrderItem_sql.num)
            else:
                checkoutRecord_materialPrice.append(0)

    records ={"时间":checkoutRecord_time, "产品":checkoutRecord_productName,"物料":checkoutRecord_materialName,"批次":checkoutRecord_materialBatch,"数量":checkoutRecord_materialCount,"金额":checkoutRecord_materialPrice} 
    print(records)
    return records
...
def genExecl(orderId):
    title = ["时间",'产品','物料','批次','数量','金额']
    records = genDatafromSql(1)
    order = SalesOrder.objects.get(pk=orderId)
    df = pd.DataFrame(records)
    df = df.reindex_axis(title,1)
